...
| 🇺🇸 Nasdaq Stock Market (`XNAS`) | Distinct US exchange identity using the shared US equities calendar supplied by `exchange_calendars`. | Pre-market, closing auction, post-market | Manual |
| 🇲🇽 Mexican Stock Exchange (`XMEX`) | Mexico's primary equities exchange. | Native regular session and holiday calendar | Manual |
| 🇨🇱 Santiago Stock Exchange (`XSGO`) | Chile's primary equities exchange. | Native regular session and holiday calendar | Manual |
